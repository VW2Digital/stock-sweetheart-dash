...
const AdminUserAddresses = ({ userId }: Props) => {
  ...
  useEffect(() => { if (userId) fetchAddresses(); /* eslint-disable-next-line */ }, [userId]);

  const resetForm = () => {
    setLabel('Casa'); setPostalCode(''); setStreet(''); setNumber('');
    setComplement(''); setDistrict(''); setCity(''); setState('');
    ...
    <Card className="md:col-span-2">
      <CardHeader>
        <div className="flex items-center justify-between gap-2">
          <CardTitle className="text-base flex items-center gap-2">
            <MapPin className="w-4 h-4" /> Endereços do Cliente
          </CardTitle>
          {!showForm && (
      ...
      <CardContent className="space-y-4">
          ...
          <Card className="border-border/50">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm">{editingId ? 'Editar endereço' : 'Novo endereço'}</CardTitle>
            ...
            <CardContent className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Input value={label} onChange={(e) => setLabel(e.target.value)} placeholder="Ex: Casa" />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <Label htmlFor="admin-addr-default" className="text-sm text-muted-foreground cursor-pointer">
        ...
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground text-center py-6">Nenhum endereço cadastrado para este cliente.</p>
        ) : (
